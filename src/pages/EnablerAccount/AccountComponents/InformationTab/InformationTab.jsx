...
function InformationTab(props) {
  ...
  const { TooltipComponent: ToolTip2, showTooltip:showTooltip2 } = useCustomToolTip();
  ...
  const [contactNum, setContactNum] = useState(profileData?.contact_number || ''); // Initialize with profileData value or empty string
  ...
  const handleClassificationChange = (value) => {
    ...
    setCurrentModal(<ModalChangeClass handleClose={handleClose} submitChangeClass={() => submitChangeClass(value)}/>);
    handleOpen();
    setError('');
  }
  ...
  const handleSaveName = async () => {
    setCurrentModal(<ModalChangeName handleClose={handleClose} submitSaveName={submitSaveName} />);
    handleOpen();
  };
  ...
  const submitPassword = async (type, value) => {
    ...
    if (type === 'newName') {
      ...
      const res = await sendRequest({
        url: `${import.meta.env.VITE_BACKEND_DOMAIN}/members/update-enablerClass`,
        method: 'POST',
        body: JSON.stringify({
          member_id: profileData.member_id,
          newClass: value,
              ...
              <div className={styles["editingContainer"]}>
                  ...
                  <section className={styles['form-container']}>
                    <h1 className={styles['editing_header']}>{changeForm ? 'Your Profile' : "Change Password"}</h1>
                          ...
                          <div className={styles['form-row']}>
                            <div className={styles['form-col']}>
                              ...
                              <TextField
                                ...
                                fullWidth
                                onChange={(e) => {
                                  handleNameChange(e.target.value); // Update name state
                                }}
                                value={name}
                                ...
                                <TextField 
                                  ...
                                  InputProps={
                                    !isEditingField('contactNum') ? {
                                      ...
                                      endAdornment: ( // Render save button only if edit is false
                                        <Button 
                                          onClick={() => handleSaveContact('contactNum')} // Set edit to true when clicked
                                          sx={{
                                            textTransform: 'none',
                                            color: '#292d3e',
                                            '&:hover': {
                              ...
                              {profileData.member_type === 2 && (
                                <Fragment>
                                  <label htmlFor="enabler_class">
                                    Classification 
                                      <ToolTip2 title={"Changing the classification of your account will have a one-year duration before changing to another classification."}>
                                        <InfoOutlined onMouseEnter={() => showTooltip2("Changing the classification of your account will have a one-year duration before changing to another classification.")}/> 
                                      </ToolTip2>
                                  </label>
                                  <Select
                                      id='enabler_class'
                                      ...
                                      fullWidth
                                      size='small'
                                      disabled={currentDate <= profileData.class_change_valid_date}
                                    >
                      ...
                      <div className={styles['form-row']}>
                        <a href='#' onClick={() => setChangeForm(prevState => !prevState)}>{changeForm ? 'Change Password' : 'Back'}</a>
                      </div>        
                  </section>
              </div>
               ) : (
...
const ModalChangeName = ({ handleClose, submitSaveName }) => (
  <Fragment>
    <section className={styles['modal-container']}>
      ...
      <p>Changing the name will have a one-year duration before changing to another name.</p>
...
const ModalChangeClass = ({handleClose, submitChangeClass}) => (
  <Fragment>
    <section className={styles['modal-container']}>
      ...
      <p>Changing the classification of your account will have a one-year duration before changing to another classification.</p>
      <div className={styles['modal-buttons']}>
        <Button
          variant='outlined'
          fullWidth
