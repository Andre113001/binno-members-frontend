      ...
      try {
        ...
        if (profileData) {
          ...
          const guidesQuery = await fetch(
            `${import.meta.env.VITE_BACKEND_DOMAIN}/posts/user/${
              profile.member_id
            }`,
            {
              headers: {
          ...
          if (Array.isArray(guidesResult)) {
            ...
            const promises = guidesResult.map(async (guide) => {
              ...
              const profilePic = await fetchImage(
              ...
              return {
                ...
                author: profileData.setting_institution,
              };
            });
                  ...
                  posts.length > 0 ? (
                    <PanelContent
                      filteredPosts={posts.filter(
                        (post) => post.post_category === "Milestone"
                      )}
                      handleDeletePost={handleDeletePost}
                    />
                  ) : (
              ...
              <TabPanel value={value} index={2}>
                {dataLoaded ? (
                  posts.length > 0 ? (
                    <PanelContent
                      filteredPosts={posts.filter(
                        (post) => post.post_category === "Promotion"
                      )}
                      handleDeletePost={handleDeletePost}
                    />
                  ) : (
